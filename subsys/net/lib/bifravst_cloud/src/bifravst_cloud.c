...
// TODO: find out where to set these defines
...
static int client_id_get(char *const id)
{
    ...
    err = at_cmd_write("AT+CGSN", imei_buf, (AWS_CLOUD_CLIENT_ID_LEN + 5),
                       NULL);
    if (err)
    {
                             ...
                             const struct mqtt_evt *mqtt_evt)
{
    ...
    case MQTT_EVT_PUBLISH:
    {
        ...
        LOG_DBG("MQTT_EVT_PUBLISH: id = %d len = %d ", p->message_id,
    ...
    case MQTT_EVT_PUBACK:
        LOG_DBG("MQTT_EVT_PUBACK: id = %d result = %d",
                mqtt_evt->param.puback.message_id, mqtt_evt->result);
        break;

    case MQTT_EVT_SUBACK:
        LOG_DBG("MQTT_EVT_SUBACK: id = %d result = %d",
                mqtt_evt->param.suback.message_id, mqtt_evt->result);
        break;
...
static int broker_init(void)
{
    ...
    struct sockaddr_in *broker = ((struct sockaddr_in *)&broker);
...
static int broker_init(void)
{
    ...
    struct addrinfo hints = {.ai_family = BIFRAVST_CLOUD_AF_FAMILY,
                             .ai_socktype = SOCK_STREAM};

    err = getaddrinfo(CONFIG_AWS_IOT_BROKER_HOST_NAME, NULL, &hints,
                      &result);
    if (err)
    {
